...
{
    public static IQueryable<Sale_Sale> Apply(IQueryable<Sale_Sale> query, SaleSearch filter)
    {
        query = query.Where(x => x.Id > 0 && x.StoreId == 1);
    ...
    public static IQueryable<SaleSummary> Project(IQueryable<Sale_Sale> query) => query.Select(x => new SaleSummary
    {
        ...
        GiveBack = x.GiveBack, OrderDate = x.DateOrder,
