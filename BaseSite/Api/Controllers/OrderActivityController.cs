...
[ApiController, Authorize(Roles = nameof(OPERATIONS.Order_Detail))]
[Route("api/orders/{id:int}/activity")]
public sealed class OrderActivityController : ControllerBase
{
    [HttpGet]
    public ActionResult<OrderActivity> Get(int id)
    {
        using var db = new PantaEntities();
        var documentNumber = db.Order_Order.AsNoTracking().Where(x => x.Id == id).Select(x => (int?)x.DocNumber).SingleOrDefault();
        if (!documentNumber.HasValue || id <= 0) return NotFound();

        var comments = db.CRM_Comments.AsNoTracking()
            .Where(x => x.TrunkTableId == (short)DB_Table.Order_Order && x.TrunkId == id)
            .OrderBy(x => x.CreateDate)
            .Select(x => new OrderCommentItem
            {
        ...
        var history = db.Log_Logs.AsNoTracking()
            .Where(x => x.EntityTableId == (int)DB_Table.Order_Order && x.EntityId == documentNumber.Value)
    ...
    public ActionResult<OrderCommentItem> AddComment(int id, OrderCommentRequest request)
    {
        ...
        using (var db = new PantaEntities())
            if (!db.Order_Order.AsNoTracking().Any(x => x.Id == id && id > 0)) return NotFound();

        var currentUser = AccountManager.Account_User_Get(userId);
        var saved = CommentManager.CRM_Comments_Edit(new CRM_Comments
        {
            TrunkTableId = (short)DB_Table.Order_Order, TrunkId = id, OwnerId = userId,
