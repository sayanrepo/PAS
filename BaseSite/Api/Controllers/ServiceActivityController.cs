...
[ApiController, Authorize(Roles = nameof(OPERATIONS.Service_Detail))]
[Route("api/services/{id:int}/activity")]
public sealed class ServiceActivityController : ControllerBase
{
    [HttpGet]
    public ActionResult<OrderActivity> Get(int id)
    {
        using var db = new PantaEntities();
        var documentNumber = db.Service_Service.AsNoTracking().Where(x => x.Id == id)
        ...
        var comments = db.CRM_Comments.AsNoTracking()
            .Where(x => x.TrunkTableId == (short)DB_Table.Service_Service && x.TrunkId == id)
        ...
        var history = db.Log_Logs.AsNoTracking()
            .Where(x => x.EntityTableId == (int)DB_Table.Service_Service && x.EntityId == documentNumber.Value)
    ...
    public ActionResult<OrderCommentItem> AddComment(int id, OrderCommentRequest request)
    {
        ...
        using (var db = new PantaEntities()) if (!db.Service_Service.AsNoTracking().Any(x => x.Id == id)) return NotFound();
        var currentUser = AccountManager.Account_User_Get(userId);
        var saved = CommentManager.CRM_Comments_Edit(new CRM_Comments
        {
            TrunkTableId = (short)DB_Table.Service_Service, TrunkId = id, OwnerId = userId,
