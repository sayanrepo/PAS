...
[ApiController, Authorize(Roles = nameof(OPERATIONS.Delivery_Detail))]
[Route("api/deliveries/{id:int}/activity")]
public sealed class DeliveryActivityController : ControllerBase
{
    [HttpGet]
    public ActionResult<OrderActivity> Get(int id)
    {
        using var db = new PantaEntities();
        var documentNumber = db.Delivery_Delivery.AsNoTracking().Where(x => x.Id == id)
        ...
        var comments = db.CRM_Comments.AsNoTracking()
            .Where(x => x.TrunkTableId == (short)DB_Table.Delivery_Delivery && x.TrunkId == id)
        ...
        var history = db.Log_Logs.AsNoTracking()
            .Where(x => x.EntityTableId == (int)DB_Table.Delivery_Delivery && x.EntityId == documentNumber.Value)
            .OrderByDescending(x => x.EventTime).Select(x => new OrderHistoryItem
            {
                Id = x.Id, EventTime = x.EventTime,
                User = (x.Account_Users.Name ?? "") + " " + (x.Account_Users.LastName ?? ""),
                Category = x.BaseSystem_Tables.Label, DocumentNumber = x.EntityId,
                Activity = x.Log_LogActivity.Name, Description = x.Description,
                IpAddress = x.IPAddress, Amount = x.LogData1
            }).ToList();
    ...
    public ActionResult<OrderCommentItem> AddComment(int id, OrderCommentRequest request)
    {
        ...
        using (var db = new PantaEntities()) if (!db.Delivery_Delivery.AsNoTracking().Any(x => x.Id == id && id > 0)) return NotFound();
        var currentUser = AccountManager.Account_User_Get(userId);
        var saved = CommentManager.CRM_Comments_Edit(new CRM_Comments
        {
            TrunkTableId = (short)DB_Table.Delivery_Delivery, TrunkId = id, OwnerId = userId,
            OwnerName = currentUser.FullName, OwnerEmail = currentUser.Email,
            Comment = request.Comment.Trim(), CreateDate = DateTime.Now
        });
