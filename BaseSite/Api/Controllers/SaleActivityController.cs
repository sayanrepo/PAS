...
[ApiController, Authorize(Roles = nameof(OPERATIONS.Sale_Detail))]
[Route("api/sales/{id:int}/activity")]
public sealed class SaleActivityController : ControllerBase
{
    [HttpGet]
    public ActionResult<OrderActivity> Get(int id)
    {
        using var db = new PantaEntities();
        var documentNumber = db.Sale_Sale.AsNoTracking().Where(x => x.Id == id && x.StoreId == 1)
        ...
        var comments = db.CRM_Comments.AsNoTracking()
            .Where(x => x.TrunkTableId == (short)DB_Table.Sale_Sale && x.TrunkId == id)
        ...
        var history = db.Log_Logs.AsNoTracking()
            .Where(x => x.EntityTableId == (int)DB_Table.Sale_Sale && x.EntityId == documentNumber.Value)
    ...
    public ActionResult<OrderCommentItem> AddComment(int id, OrderCommentRequest request)
    {
        ...
        using (var db = new PantaEntities())
            if (!db.Sale_Sale.AsNoTracking().Any(x => x.Id == id && x.StoreId == 1)) return NotFound();
        var currentUser = AccountManager.Account_User_Get(userId);
        var saved = CommentManager.CRM_Comments_Edit(new CRM_Comments
        {
            TrunkTableId = (short)DB_Table.Sale_Sale, TrunkId = id, OwnerId = userId,
